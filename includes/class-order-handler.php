 ...
 * @since 2.1.0
 */
class WCV_SC_Order_Handler extends WCV_SC_Payment_Gateway {
	...
	public function __construct() {
		...
		add_filter( 'wcv_sc_tracks_event_properties', [ $this, 'wcv_sc_tracks_event_properties' ], 10, 2 );
	}
	...
	public function process_redirect_payment( $order_id, $retry = true, $previous_error = false ) {
		try {
			...
			WCV_SC_Logger::log( "Info: (Redirect) Begin processing payment for order $order_id for the amount of {$order->get_total()} by source {$source}" );

			/**
			 * First check if the source is chargeable at this time. If not,
			 * webhook will take care of it later.
			 */
			$source_info = Source::retrieve( $source );
			
			if ( ! empty( $source_info->error ) ) {
				throw new WCV_SC_Exception( print_r( $source_info, true ), $source_info->error->message );
			}

			if ( 'failed' === $source_info->status || 'canceled' === $source_info->status ) {
				throw new WCV_SC_Exception( print_r( $source_info, true ), __( 'Unable to process this payment, please try again or use alternative method.', 'wc-vendors-gateway-stripe-connect' ) );
			}
			...
			// Make the request.
			$response = Charge::create( $this->generate_payment_request( $order, $prepared_source ) );
			...
			if ( ! empty( $response->error ) ) {
				...
				if ( $this->is_no_such_token_error( $response->error ) && $prepared_source->token_id ) {
					...
					$wc_token = WCV_SC_Payment_Tokens::get( $prepared_source->token_id );
					$wc_token->delete();
					$localized_message = __( 'This card is no longer available and has been removed.', 'wc-vendors-gateway-stripe-connect' );
				...
				throw new WCV_SC_Exception( print_r( $response, true ), $message );
			}

			do_action( 'wc_gateway_stripe_process_redirect_payment', $response, $order );
			$response->balance_transaction = $response->balance_transaction->id;
			$charges = array(
				1 => $response,
			);

			$this->process_response( (object) array( 'charges' => $charges ), $order );
			...
			$order->update_status( 'failed', sprintf( __( 'Stripe payment failed: %s', 'wc-vendors-gateway-stripe-connect' ), $e->getLocalizedMessage() ) );
	...
	public function capture_payment( $order_id ) {
		WCV_SC_Logger::log( 'capture_payment');
		$order = wc_get_order( $order_id );
			...
			if ( $charge && 'no' === $captured ) {
				...
				if ( $intent ) {
					...
					if ( ! empty( $intent->error ) ) {
						/* translators: error message */
						$order->add_order_note( sprintf( __( 'Unable to capture charge! %s', 'wc-vendors-gateway-stripe-connect' ), $intent->error->message ) );
					} elseif ( 'requires_capture' === $intent->status ) {
						WCV_SC_Logger::log( '$intent->status: ' . $intent->status);
						$level3_data = $this->get_level3_data_from_order( $order );
						$result      = PaymentIntent::capture(
							[
								'id'       => $intent->id,
								'amount'   => WCV_SC_Helper::get_stripe_amount( $order_total ),
						...
						if ( ! empty( $result->error ) ) {
							/* translators: error message */
							$order->update_status( 'failed', sprintf( __( 'Unable to capture charge! %s', 'wc-vendors-gateway-stripe-connect' ), $result->error->message ) );
						} else {
					...
					$result = Charge::retrieve( $charge );

					if ( ! empty( $result->error ) ) {
						/* translators: error message */
						$order->add_order_note( sprintf( __( 'Unable to capture charge! %s', 'wc-vendors-gateway-stripe-connect' ), $result->error->message ) );
					} elseif ( false === $result->captured ) {
						WCV_SC_Logger::log( '$result->captured: ' . $result->captured);
						$level3_data = $this->get_level3_data_from_order( $order );
				...
				if ( $is_stripe_captured ) {
					...
					$order->add_order_note( sprintf( __( 'Stripe charge complete (Charge ID: %s)', 'wc-vendors-gateway-stripe-connect' ), $result->id ) );
					...
					WCV_SC_Logger::log( '$result->balance_transaction->id: ' . $result->balance_transaction->id);
	 ...
	 */
	public function wcv_sc_tracks_event_properties( $properties, $prefixed_event_name ) {
